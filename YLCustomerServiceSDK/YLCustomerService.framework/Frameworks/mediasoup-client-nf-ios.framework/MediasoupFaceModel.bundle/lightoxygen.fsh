...
void main()
{
    ...
    rgba = mix(sourceColor, textureColor, 0.8);
	
	redCurveValue = clamp(rgba.r*0.8333,0.0,1.0 );
	greenCurveValue = clamp(rgba.g*0.84,0.0,1.0  );
	blueCurveValue = clamp(rgba.b*0.8866,0.0,1.0 );
	
	rgba=vec4(redCurveValue,greenCurveValue,blueCurveValue,1.0);
	
//	vec4 metal = texture2D(metalTexture, textureCoordinate);
	
    gl_FragColor = mix(rgba,sourceColor,0.8);
} 
