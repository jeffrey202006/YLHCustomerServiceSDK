...
void main()
{
    ...
    rgba = mix(sourceColor, textureColor, 0.7);
	
	redCurveValue = clamp(rgba.r*0.86,0.0,1.0 );
	greenCurveValue = clamp(rgba.g*0.93333,0.0,1.0  );
	blueCurveValue = clamp(rgba.b*0.99333,0.0,1.0 );
	
	rgba=vec4(redCurveValue,greenCurveValue,blueCurveValue,1.0);
	
//	vec4 metal = texture2D(metalTexture, textureCoordinate);
	
    gl_FragColor = mix(rgba,sourceColor,0.7);
} 
