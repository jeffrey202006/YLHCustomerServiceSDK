...
void main() {
	...
	highp vec4 sourceColor = texture2D(inputImageTexture, textureCoordinate.xy);
	...
	textureColor = vec4(clamp(redCurveValue*0.98666,0.0,1.0), clamp(greenCurveValue*0.95,0.0,1.0), clamp(blueCurveValue*1.0,0.0,1.0), 1.0);

    gl_FragColor = mix(sourceColor, textureColor, 1.0);
}
